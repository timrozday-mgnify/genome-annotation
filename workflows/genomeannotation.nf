...
workflow GENOMEANNOTATION {
    ...
    emit:
    cds_locations               = cdss
    pfam_annotations            = params.run_pfam   ? PFAM_CONCATENATE.out.concatenated_file             : Channel.empty()
    dbcan_overview_annotations  = params.run_dbcan  ? DBCAN_OVERVIEW_CONCATENATE.out.concatenated_file   : Channel.empty()
    dbcan_cazyme_annotations    = params.run_dbcan  ? DBCAN_CAZYME_CONCATENATE.out.concatenated_file     : Channel.empty()
    dbcan_substrate_annotations = params.run_dbcan  ? DBCAN_SUBSTRATE_CONCATENATE.out.concatenated_file  : Channel.empty()
    dbcan_diamond_annotations   = params.run_dbcan  ? DBCAN_DIAMOND_CONCATENATE.out.concatenated_file    : Channel.empty()
    // kofam_tsv_annotations = KOFAM_TSV_CONCATENATE.out.concatenated_file
    kofam_txt_annotations       = params.run_kofam  ? KOFAM_TXT_CONCATENATE.out.concatenated_file        : Channel.empty()
